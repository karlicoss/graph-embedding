...
public class Graph {
    public String toString()
    {
        ...
        for (int i = 0; i < size(); i++)
        {
            for (int j = i + 1; j < size(); j++)
            {
                if (m[i][j] == 1)
                {
                    ans += Integer.toString(getLabel(i)) + "->" + Integer.toString(getLabel(j)) + "; ";
                }
            }
        }
        return ans;
    ...
    private void layChain(boolean[][] laide, ArrayList<Integer> chain, boolean cyclic)
    {
    ...
    private boolean containsSegment(final ArrayList<Integer> face, final Graph segment, boolean[] laidv)
    {
        for (int f = 0; f < size(); f++)
        {
            for (int t = 0; t < size(); t++)
            {
                ...
                {
                    if ((laidv[f] && !face.contains(f)) || (laidv[t] && !face.contains(t)))
                    {
                        return false;
                    }
                }
    ...
    public Pair<ArrayList<ArrayList<Integer>>, ArrayList<Integer>> getEmbedding()
    {
        if (size() == 1)
        {
            ...
            return new Pair<ArrayList<ArrayList<Integer>>, ArrayList<Integer>>(faces, face);
        }
        ...
        while (true)
        {
            ...
            int mi = 0; // Searching for the segment which can be embedded in the minimal number of faces
            for (int i = 0; i < segments.size(); i++) {
                if (count[i] < count[mi])
                    mi = i;
            }
            ...
            else
            {
                ...
                if (face != outerFace)
                {
                    ...
                    {
                        face1.addAll(chain);
                        for (int i = (chfn + 1) % fsize; i != chst; i = (i + 1) % fsize)
                        {
                            face1.add(face.get(i));
                        }
                        face2.addAll(rchain);
                        for (int i = (chst + 1) % fsize; i != chfn; i = (i + 1) % fsize)
                        {
                ...
                else
                {
                    ...
                    if (chst < chfn)
                    {
                        ...
                        face2.addAll(chain);
                        for (int i = (chfn - 1 + fsize) % fsize; i != chst; i = (i - 1 + fsize) % fsize)
                        {
                            face2.add(face.get(i));
                        }
                    }
                    else
                    {
                        ...
                        face2.addAll(rchain);
                        for (int i = (chst - 1 + fsize) % fsize; i != chfn; i = (i - 1 + fsize) % fsize)
                        {
                            face2.add(face.get(i));
                        }
                    }
        ...
        return new Pair<ArrayList<ArrayList<Integer>>, ArrayList<Integer>>(faces, outerFace);
    }
    ...
    private void dfsChain(int used[], boolean[] laidv, ArrayList<Integer> chain, int v)
    {
        used[v] = 1;
        chain.add(v);
        for (int i = 0; i < size(); i++)
    ...
    private void dfsPaintBD(int[] enter, int[] ret, int[] colors, int curColor, MutableInteger maxColor, ArrayList<Edge> bridges, int v)
    {
        colors[v] = curColor;
        for (int i = 0; i < size(); i++)
        {
            if (existsEdge(v, i))
            {
                if (colors[i] == 0)
                {
                    if (enter[i] == ret[i])
                    {
                        ...
                        maxColor.increment();
                        dfsPaintBD(enter, ret, colors, maxColor.getValue(), maxColor, bridges, i);
                    }
                    else
                    {
                        dfsPaintBD(enter, ret, colors, curColor, maxColor, bridges, i);
                    }
                }
            }
        }
    ...
    private void dfsCPD(int[] used, int[] enter, int[] ret, MutableInteger time, int p, int v)
    {
        time.increment();
        enter[v] = time.getValue();
        ret[v] = enter[v];
    ...
    public void dfsPaintCPD(int[] used, int[] enter, int[] ret, int curColor, MutableInteger maxColor, int[][] color, boolean[] isCutPoint, int p, int v)
    {
        used[v] = 1;
            ...
            if (existsEdge(v, i))
            {
                ...
                if (used[i] == 0)
                {
                    ...
                    if (ret[i] >= enter[v])
                    {
                        ...
                        if (p != -1)
                            isCutPoint[v] = true;
                        dfsPaintCPD(used, enter, ret, maxColor.getValue(), maxColor, color, isCutPoint, v, i);
                    }
                    else
                    {
                        color[v][i] = curColor;
                        dfsPaintCPD(used, enter, ret, curColor, maxColor, color, isCutPoint, v, i);
                    }
                }
                else
                {
    ...
    public Pair<ArrayList<Graph>, ArrayList<Integer>> getCutPointsDecomposition()
    {
        ...
        if (size() == 1)
        {
            ...
            return new Pair<ArrayList<Graph>, ArrayList<Integer>>(vertexConnectedComponent, cutPoints);
        }
        ...
        for (int i = 0; i < size(); i++)
        {
            if (used[i] == 0)
            {
                dfsPaintCPD(used, enter, ret, 0, maxColor, color, isCutPoint, -1, i);
            }
        }
